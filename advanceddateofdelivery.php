...
class AdvancedDateOfDelivery extends Module implements WidgetInterface
{
	...
	public function install()
	{
		...
		if (!parent::install()
			...
			|| !$this->registerHook('displayReassuranceCart')
	...
	public function uninstall()
	{
		if (!parent::uninstall()
			...
			|| !$this->unregisterHook('displayReassuranceCart')
	...
	*/
	...
	public function displayForm()
	{
		...
		if (version_compare(_PS_VERSION_, '1.8.0', '>='))
			$helper->fields_value['ADOD_PRODUCT_PAGE_TXT'] = Configuration::getConfigInMultipleLangs('ADOD_PRODUCT_PAGE_TXT');
		else
		...
		if (version_compare(_PS_VERSION_, '1.8.0', '>='))
			$helper->fields_value['ADOD_DATE_FORMAT'] = Configuration::getConfigInMultipleLangs('ADOD_DATE_FORMAT');
		else
	...
	*/
	...
	public function hookdisplayHeader($params)
	{
		...
		if($this->context->controller->php_self == 'cart'){
			//$this->context->controller->registerJavascript('modules-advanceddateofdelivery-2', 'modules/'.$this->name.'/views/js/product_17_cart.js', array('position' => 'bottom', 'priority' => 150));
		}
	...
	*/
		...
		if (Configuration::get('ADOD_ALLOW_ASAP_OPTION'))
		{
			...
			foreach ($dates_delivery as $dates)
			{
				...
				if ($dates['date_min'] == $dates['date_max'])
					$return .= sprintf($this->l('%1$s:'), $dates["product_datas"]["product_name"]).' '.sprintf($this->l('Approximate date of delivery on %1$s.'), $dates['date_min']).'<br/>';
				else
					$return .= sprintf($this->l('%1$s:'), $dates["product_datas"]["product_name"]).' '.sprintf($this->l('Approximate date of delivery is between %1$s and %2$s.'), $dates['date_min'], $dates['date_max']).'<br/>';
			}
		}
		else
		{
	...
	public function hookBeforeCarrier($params)
	{
		...
		{
			$this->smarty->assign(array(
				'cartDatesDelivery' => json_encode($dates_delivery),
			));
    ...
    {   
        ...
        /**
         * Actions de changement de template mail sur le formulaire order_conf
         */
        if ($params['template'] == 'order_conf') {
            $params['templatePath'] = _PS_MODULE_DIR_.$this->name . '/mails/';
            // Champs du template mail mis à jour directement dans le module (réutilisation des fonctions)
            // $params['templateVars']['{estimated_delivery_date}'] = $estimatedDeliveryDate;
        }
    }
	...
	public function renderWidget($hookName, array $params)
	{
		...
		// Empêche l'affichage des widgets si le module n'est pas actif pour la boutique actuelle
		...
		switch ($hookName) {
			case 'displayShoppingCartFooter':
				if ($this->context->controller->php_self == 'cart') {
					...
					$carrierList = $this->getProductCarriersCart($params['product']['id_product']);
			
					$adod_product_page_txt = Configuration::get('ADOD_PRODUCT_PAGE_TXT', (int)Context::getContext()->language->id);
			
					foreach($carrierList as $carrier) {
						$id_carrier = $carrier['id_carrier'];
						foreach($carrier['delivery_date'][0][$id_carrier] as $key=>$delivery_date) {
			
							if($key == $id_product."_".$id_product_attribute) {
								$date_min = $delivery_date[0]['date_min'];
								$date_max = $delivery_date[1]['date_max'];
							}
						}
					}
					...
					$this->smarty->assign(array(
						...
						//'embedded_attributes' => $params['product']['embedded_attributes'],
						'carrier_delivery_list' => $carrierList,
						'adod_product_position' => Configuration::get('ADOD_PRODUCT_POSITION'),
						'adod_product_display' => Configuration::get('ADOD_PRODUCT_DISPLAY'),
						'adod_product_page_txt' => str_replace('%MINIMAL_DATE%', $date_min, $adod_product_page_txt),
						'display_carrier_price' => Configuration::get('ADOD_DISPLAY_CARRIER_PRICE')
					));
					return $this->display(__FILE__, '/product_actions_cart.tpl');
				}
			case 'displayOrderDetail':
				//if ($this->context->controller->php_self == 'cart') {
					//print_r($params);exit;
					$id_product = $params['product']['product_id'];
					
					$id_product_attribute = $params['product']['product_attribute_id'];

					$carrierList = $this->getProductCarriersCart($id_product);
		...
		//$result = $this->getWidgetVariables($hookName, $params);
	...
	public function hookDisplayReassuranceCart($params)
	{
		//print_r($params['product']['id_product']);
		//print_r($params['product']['id_product_attribute']);
		//print_r($params['product']['id_product']);
		//exit;
	...
	/**
	 * Add specific delivery date to confirmation mail
	 *
	 * @param ['template','template_vars'] $params
	 */
	public function hookActionGetExtraMailTemplateVars($params)
	{
		...
		if($params["template"] == "order_conf"){
			...
			if(array_key_exists("{id_order}", $params["template_vars"])){
				$date_of_delivery = $this->getOrderDateOfDelivery($params["template_vars"]["{id_order}"]);
			}elseif(array_key_exists("cart", $params)){
				...
				if ($date_min == $date_max){
					$date_of_delivery = sprintf($this->l('Approximate date of delivery on %1$s.'), $date_min);
				}
				else{
					$date_of_delivery = sprintf($this->l('Approximate date of delivery is between %1$s and %2$s.'), $date_min, $date_max);
				}

			}

			// Send to template
			$params['extra_template_vars']['{estimated_delivery_date}'] = $date_of_delivery;
	...
	public function getProductCarriersCart($id_product)
	{
		// Variables
		//$id_product = (int)Tools::getValue('id_product');
	...
	public function getAvailableDate($id_product, $id_product_attribute = null)
    {
        ...
        if ($id_product_attribute === null) {
			...
			// Suppression de la condition car renvoyais la date du produit par défaut même si la déclinaison n'en a pas
